...
export default function LostAndFoundFeed() {
  ...
  const lostEntries = publicEntries.filter(({ type }) => type === "lost_item_post");
  const foundEntries = publicEntries.filter(({ type }) => type !== "lost_item_post");

  return <section className="exchange-feed lost-and-found" aria-labelledby="lost-found-title">
    <FeedHeader
      eyebrow="SEARCH CAMPUS"
      title="Lost & Found"
      id="lost-found-title"
      countLabel={`${publicEntries.length} visible`}
      description="Tell us what happened, or search reports from around campus."
    />

    <section className="exchange-panel report-choice" aria-labelledby="report-choice-title">
    ...
    /> : <aside className="exchange-panel lost-item-sign-in"><h3>Sign in to report what you lost</h3><p>Use your uNivUS session to create a private, Moderator-reviewed post.</p><a href="/univus/">Open through uNivUS</a></aside>)}
    ...
    <div className="lost-item-grid" aria-label="Lost property">
      {lostEntries.map(({ value }) => <LostItemCard key={value.id} post={value} />)}
      {!lostEntries.length && <p className="feed-state" aria-live="polite">No Lost-Item Posts match these filters.</p>}
    </div>

    <section className="found-item-workflow" aria-labelledby="found-property-title">
      <FeedHeader
        eyebrow="FOUND PROPERTY"
        title="Found property"
        id="found-property-title"
        countLabel={`${foundEntries.length} visible`}
      />
      <div className="lost-item-grid">
        {foundEntries.map((entry) => <FoundPropertyCard key={`${entry.type}-${entry.value.id}`} value={entry.value} type={entry.type} />)}
        {!foundEntries.length && <p className="feed-state" aria-live="polite">No Found Property matches these filters.</p>}
      </div>
    </section>
  </section>;
}
...
function LostItemWorkflowForm({ draft, setDraft, setFiles, editingId, mine, busy, onSubmit, onReset, onEdit, onWithdraw }) {
  return <section className="participant-property-workflow" aria-labelledby="lost-item-form-title">
    <form id="lost-item-form" className="exchange-panel lost-item-form" onSubmit={onSubmit}>
    ...
    <section className="exchange-panel my-lost-items" aria-labelledby="my-lost-items-title"><h2 id="my-lost-items-title">My lost-item posts</h2>{mine.length ? <ul>{mine.map((post) => <li key={post.id}><div><strong>{post.category} · {post.lostDate}</strong><span className={`status-badge status-${post.status}`}>{post.status.replaceAll("_", " ")}</span></div><p>{post.description}</p>{post.rejectionReason && <p className="rejection-reason">Moderator reason: {post.rejectionReason}</p>}<small>Revision {post.revision} · ownership details retained privately</small><div className="management-actions">{["pending_review", "rejected"].includes(post.status) && <button className="secondary-action" type="button" onClick={() => onEdit(post)}>Edit post</button>}{post.status !== "withdrawn" && <button className="danger-action" type="button" onClick={() => onWithdraw(post.id)}>Withdraw</button>}</div></li>)}</ul> : <p>No lost-item posts yet.</p>}</section>
  </section>;
}

function LostItemCard({ post }) {
  return <article className="exchange-card listing-card lost-item-card">
